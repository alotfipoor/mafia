...
export default function RoleInquiryPanel() {
  ...
  const handleRoleInquiry = () => {
    ...
    addToGameLog(`${player.name}'s role (${player.role.name}) was revealed by role inquiry. ${gameState.zodiacScenario!.roleInquiriesLeft - 1} inquiries remaining.`);
  ...
  return (
    <div className="fixed top-4 right-4 z-10">
      {!showPanel ? (
        ...
        >
          Role Inquiry ({gameState.zodiacScenario.roleInquiriesLeft})
        </button>
      ) : (
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-xl border border-purple-300 dark:border-purple-800 max-w-sm w-full">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-purple-600 dark:text-purple-400">Role Inquiry</h2>
            <button
          ...
          <div className="mb-4">
            <p className="text-gray-600 dark:text-gray-300 mb-2">
              You have {gameState.zodiacScenario.roleInquiriesLeft} role inquiries left. 
